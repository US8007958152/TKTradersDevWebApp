$(document).ready(function () {
   // loadProducts();
    ...
    //loadTrucks();
    loadStocks();
    $("#DriverName,#DriverMobile,#TruckNumber").closest('.j-unit').hide();
...
function validateForm() {
    ...
    if (parseFloat($("#hdnAvailableStock").val()) < parseFloat($("#Quantity").val())) {
        $("#Quantity").closest('.j-input').removeClass('j-success-view').find('span').remove();
...
function getOrderById(orderId) {
    //$.ajax({
    //    url: "/Inventories/Order/GetOrderById?orderId=" + orderId,
    //    type: "GET",
    //    dataType: "json",
    //    beforeSend: function () {
    ...
    //    success: function (response) {
    //        if (response.statusCode > 0) {
    //            $("#ddlUserType").val(response.data.userTypeId);
    //            showOrHideCustomerDetails();
    //            $("#ddlTransportType").val(response.data.transportTypeId);
    //            if (response.data.userTypeId == 3) {
    //                $("#ddlDealer").val(response.data.userId);
    //            }
    //            else {
    //                $("#CustomerName").val(response.data.customerName);
    //                $("#CustomerMobile").val(response.data.customerMobile);
    //                $("#CustomerAddress").val(response.data.customerAddress);
    //            }               
                
    //            $("#Quantity").val(response.data.tOrderDetails[0].quantity);
    //            $("#OrderDate").val(response.data.orderDate);
    //            $("#ddlPaymentType").val(response.data.paymentTypeId);
    //            $("#ddlStatus").val(response.data.statusTypeId);
    //            $("#ddlDriver").val(response.data.driverId);
    //            $("#ddlTruck").val(response.data.truckId);
    //            $("#TotalAmount").val(response.data.totalAmount);
    //            $("#PaidAmount").val(response.data.paidAmount);
    //            $("#Comments").val(response.data.comments);
    //            $("#ddlProduct").val(response.data.tOrderDetails[0].productId);
    //            loadProductTypes(response.data.tOrderDetails[0].productTypeId);
