$(document).ready(function () {
    ...
    //    if (parseFloat($("#hdnAvailableStock").val()) < parseFloat($(this).val()))
    //    {
...
function showOrHideSupplierDetails(type) {
    ...
    if ($("#ddlSupplier").val() == '-111') {

        $("#productTotalAmount,#productPaidAmount").closest('.j-unit').show();
        $("#productTotalAmount").attr('required', true);
    }
    else {
        $("#productTotalAmount,#productPaidAmount").val('');
        $("#productTotalAmount").removeAttr('required');
        $("#productTotalAmount,#productPaidAmount").closest('.j-unit').hide();
    }
}
...
function showOrHideTruckDetails(type) {
    if (type.toUpperCase() == 'HIDE') {
        $("#TruckOwnerMobile,#TruckOwnerName,#TruckNumber,#TruckRent").val('');
        $("#TruckOwnerMobile,#TruckOwnerName,#TruckNumber,#TruckRent").removeAttr('required');
        $("#TruckOwnerMobile,#TruckOwnerName,#TruckNumber,#TruckRent").closest('.j-unit').hide();
    }
    else {
        $("#TruckOwnerMobile,#TruckOwnerName,#TruckNumber,#TruckRent").attr('required', true);
        $("#TruckOwnerMobile,#TruckOwnerName,#TruckNumber,#TruckRent").closest('.j-unit').show();
    }
}
...
function loadStocks() {
    if ($("#ddlSupplier").val() == "-111" && $("#ddlProduct").val() != "" && $("#ddlProductType").val() != "") {
        $.ajax({
            ...
            success: function (response) {
                if (response.statusCode > 0) {
                    ...
                    if (availableStock > 0) {
                        ...
                        toastr.info('Available stock for ' + $("#ddlProductType option:selected").text() + ' ' + $("#ddlProduct option:selected").text()+' is : ' + stock.Table[0].Stock);
                    }
                    else {
...
function createTransport() {
    ...
    if ($("#hdnTransportId").length > 0) {
        ...
        apiUrl = "/Transports/Transport/Create";
    var formData = {
